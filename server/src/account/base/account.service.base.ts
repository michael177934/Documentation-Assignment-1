...
export class AccountServiceBase {
  ...
  async getBeneficiaries(parentId: string): Promise<Beneficiary | null> {
  ...
  async getNotifications(parentId: string): Promise<Notification | null> {
    return this.prisma.account
      .findUnique({
        where: { id: parentId },
      })
  ...
  async getReceiverAccountId(parentId: string): Promise<Transaction | null> {
    return this.prisma.account
      .findUnique({
        where: { id: parentId },
      })
  ...
  async getTransactions(parentId: string): Promise<Transaction | null> {
    return this.prisma.account
      .findUnique({
        where: { id: parentId },
      })
